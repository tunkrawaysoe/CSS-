...
products.forEach(product => {
  productHtml += `
    <div class="group rounded-sm overflow-hidden relative bg-white cursor-pointer js-product-card" data-id="${product.id}">
      <div class="relative">
        <img src="${product.image}" class="w-full h-60 object-cover" alt="${product.name}">
        <button class="absolute top-3 right-3 w-9 h-9 rounded-full bg-white shadow js-wishlist-btn">
          <i class="bi bi-heart"></i>
        </button>
        <button class="js-add-to-cart-btn absolute bottom-3 left-3 w-10 h-10 rounded-full bg-white shadow" data-button-id="${product.id}">
